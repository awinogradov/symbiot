#!/usr/bin/env bun
/**
 * `symbiot-gemini` CLI entrypoint. Dispatches to:
...
const usage = (): never => {
  process.stderr.write(
    "usage: symbiot-gemini <install-hook|uninstall-hook|run-hook|annotate <file.md>>\n"
  );
  process.exit(64);
};
...
const runInstall = async (): Promise<number> => {
  ...
  process.stdout.write(
    `installed symbiot-gemini AfterAgent hook in ${result.path}\n  command: ${result.command}\n`
  );
  return 0;
};
...
const runUninstall = async (): Promise<number> => {
  ...
  process.stdout.write(
    result.removed === 0
      ? `no symbiot-gemini hook found in ${result.path}\n`
      : `removed ${result.removed} symbiot-gemini hook entr${result.removed === 1 ? "y" : "ies"} from ${result.path}\n`
  );
  return 0;
};
