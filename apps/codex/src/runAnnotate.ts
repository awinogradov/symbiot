/**
 * `symbiot-codex annotate <file.md>` — boot the viewer in annotate mode against
 * the given markdown file under the `codex` storage namespace. Blocks until the
...
export const runAnnotate = async (filePath: string | undefined): Promise<number> => {
  if (filePath === undefined) {
    process.stderr.write("usage: symbiot-codex annotate <file.md>\n");
    return 64;
  }
  ...
  return runPlanReview({
    plan,
    mode: "annotate",
    serverOptions: { indexHtmlGz: viewerHtmlGz, agentId: "codex" },
    onStart: (url) => process.stderr.write(`symbiot-codex: annotate ${filePath} at ${url}\n`),
    onResolved: (decision) => {
