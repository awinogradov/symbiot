...
- **`dark:` variant is class-based, not media-query-based.** `packages/tailwind-config/theme.css` overrides Tailwind v4's `prefers-color-scheme` default with `@custom-variant dark (&:where(.dark, .dark *))` so the variant tracks `ThemeProvider`'s `.dark` toggle on `<html>` and an explicit user-selected Light overrides OS Dark (FR-14.3). Without this, `dark:`-prefixed utilities — most visibly `dark:prose-invert` on the editor wrapper — activate from OS preference alone, flipping `--tw-prose-*` to invert-mode values on a light background.
