...
| 3 | 🟡 In progress | [`03-critical-features.md`](./03-critical-features.md) | The 3 plan-review annotation types (Comment, Global Comment, Deletion), Review + Redline modes, sidebar, feedback export. **Split into four sub-phases**, executed in order: |
| 3.0 | ✅ Complete (2026-05-19) | [`03-0-playwright-bdd.md`](./03-0-playwright-bdd.md) | Playwright-BDD harness landed. 14/14 scenarios green; conventions documented in `features/README.md`. |
| 3.1 | 🟡 Partial (2026-05-19) | [`03-1-wire-format-and-markdown.md`](./03-1-wire-format-and-markdown.md) | Codec (C/G/D) + dual-anchor + walker + CommentComposer wire-in + GFM (lists, strikethrough) + `@tailwindcss/typography` shipped. **Outstanding:** Shiki on code blocks, `SourceLinesPlugin`, explicit table-rules wiring, golden-fixture capture. |
| 3.2 | 🟡 M2 gate (2026-05-19) | [`03-2-deletion-modes-sidebar.md`](./03-2-deletion-modes-sidebar.md) | Deletion authoring + Global Comment composer + M2 gate Playwright scenario landed. **Outstanding:** `RedlineEditor` + mode toggle, sidebar + 7 shadcn primitives, strikethrough rendering for deletion marks. |
| 3.3 | 🟡 Server complete (2026-05-19) | [`03-3-images-drafts-annotate.md`](./03-3-images-drafts-annotate.md) | `/api/upload` + `/api/image` (security model) + `/api/draft` + `/api/feedback` + drafts hook + `symbiot annotate <file.md>` CLI shipped. **Outstanding:** `@platejs/media` + composer image-attach UI. |
