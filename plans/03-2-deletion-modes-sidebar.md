...
> **Status:** 🟢 Complete (2026-05-19). The M2 gate scenario (`features/plan-review/m2-gate.feature`) drops one Comment, one Deletion, and one Global Comment and asserts all three appear in the feedback markdown. Deletion authoring (mark-only `suggestion: true` + `suggestion_<id>: true`) + top-bar Global Comment composer landed in the first iteration. This iteration closes the follow-ups: `RedlineEditor` (explicit Delete/Backspace keystroke on a non-empty selection — not 400 ms debounced), `ToggleGroup` Review/Redline in `TopBar` persisted via `localStorage["symbiot.editor-mode"]`, `AnnotationSidebar` (Tabs / Badge / AlertDialog clear-all) + the seven shadcn primitives (`Sidebar`, `ToggleGroup`, `DropdownMenu`, `Tabs`, `Badge`, `AlertDialog`, `Tooltip`), and a `SuggestionMarkPlugin` leaf renderer that wraps deletion-marked text in `<s class="line-through">`.
...
- [x] **Comment**, **Global Comment**, and **Deletion** can all be applied in the viewer and survive a `serializeFeedback` round-trip. Byte-equality goldens for `comment.md`, `global-comment.md`, `deletion.md`, `mixed.md` live in `fixtures/plannotator-reference/` and are pinned by `serializeFeedback.test.ts`; the M2 gate Playwright scenario continues to use `toContain` so a small UI text edit doesn't break it.
...
- [x] Redline mode: making a selection and pressing Delete/Backspace applies a Deletion. *(Per user direction the debounced auto-deletion + "Undo last redline" history were descoped — explicit keystroke needs no undo.)*
- [x] Mode toggle in the top bar persists across reloads via `localStorage["symbiot.editor-mode"]`. `ToggleGroup` (Review / Redline) added to `TopBar`.
- [x] Right-aligned collapsible sidebar with Comments / Deletions / Globals tabs, `Badge` counts, click-to-focus (via `data-anno-id` DOM selector + `scrollIntoView`), Clear-all `AlertDialog`.
- [~] Top bar — `DropdownMenu` action consolidation was descoped per user direction. Flat Approve / Request changes / Global Comment buttons remain; the `DropdownMenu`, `Tooltip`, and remaining primitives still ship for Phase 4+ use.
- [x] All missing shadcn primitives vendored under `packages/symbiot-ui/src/components/`: `Sidebar`, `ToggleGroup`, `DropdownMenu`, `Tabs`, `Badge`, `AlertDialog`, `Tooltip` (the last reuses `@radix-ui/react-tooltip` already in the dep tree).
- [x] All four golden fixtures (`comment.md`, `global-comment.md`, `deletion.md`, `mixed.md`) byte-match in `serializeFeedback.test.ts`. The M2 gate Playwright scenario uses `toContain` so cosmetic UI edits don't churn the e2e suite — codec-level byte-equality is the vitest test's job.
