Feature: Annotate mode — add a global comment
  ...
  top bar, types a body, and submits. The recorded annotate feedback emits a
  'General feedback' section with the quoted body.

  Scenario: Drop a global comment via the top-bar composer in annotate mode
