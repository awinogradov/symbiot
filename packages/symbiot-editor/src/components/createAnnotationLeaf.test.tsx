...
describe("createAnnotationLeaf", () => {
  it("renders the given tag with the given className and passes attributes through", () => {
    const Leaf = createAnnotationLeaf("mark", "test-class rounded", "TestLeaf");
  ...
  it("sets the React displayName so React DevTools surface the kind", () => {
    const Leaf = createAnnotationLeaf("s", "x", "ScratchedLeaf");
  ...
  it("renders with different tag names per call so each annotation kind gets its own semantic element", () => {
    const Ins = createAnnotationLeaf("ins", "insert", "Ins");
    const Del = createAnnotationLeaf("s", "delete", "Del");
